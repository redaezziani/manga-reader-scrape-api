...
export const getLatestMangaList = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await redisClient.set(cacheKey, JSON.stringify(mangaList.data), { EX: 60 * 60 }); // Cache for 1 hour
    res.status(200).json({
      status: "success",
      statusText: "Latest manga list",
      data: mangaList,
...
export const getMangaDetails = async (req: Request, res: Response) => {
  ...
  if (!source) {
    ...
    try {
        ...
        await redisClient.set(cacheKey, JSON.stringify(mangaDetails.data), { EX: 60 * 60 }); // Cache for 1 hour
        res.status(200).json({
            status: "success",
            statusText: "Manga details",
